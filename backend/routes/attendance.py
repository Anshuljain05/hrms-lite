...
def mark_attendance(attendance: AttendanceCreate, db: Session = Depends(get_db)):
    ...
    if not employee:
        raise HTTPException(
            ...
            detail=f"Employee with ID '{attendance.employeeId}' not found"
        )

    # Check if attendance already exists for this employee and date
    existing_attendance = db.query(Attendance).filter(
        (Attendance.employeeId == attendance.employeeId) & (Attendance.date == attendance.date)
    ).first()

    if existing_attendance:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail=f"Attendance record already exists for employee '{attendance.employeeId}' on {attendance.date}"
        )
...
def get_employee_attendance(employee_id: str, db: Session = Depends(get_db)):
    ...
    records = db.query(Attendance).filter(Attendance.employeeId == employee_id).all()
    return {
        "records": records,
        "total": len(records)
    }
